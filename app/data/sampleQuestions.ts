...
const unsortedSampleQuestions: Question[] = [
...

  {
    id: 'sample-reading',
    type: '閱讀測驗',
    article: 'Teacoo，發音如中文的「題庫」(彙整題目的資料庫)，初衷是要Make teachers cool. 你也知道，老師們真正發光發熱的時候是在講課時或是替學生解答疑惑時，而不是在電腦前拼命出題目、整理文件、改考卷、算成績，因此，何不利用科技的力量，讓老師們利用一杯tea的時間完成這些麻煩事呢？只要把題目丟給Teacoo，AI就可以在幾秒內幫你建立好專屬於你的題目，甚至可以派發這些題目給學生，學生無需帳號密碼，只要點開連結就能作答，系統還會自動幫你收卷、對答案、算成績，這樣該有多好？\n因此，Teacoo就此誕生，它不只是老師們的最強助教，也是學生們的最佳學伴，有了Teacoo，即使身為學生也可以幫自己整理各種題目，不管是課堂題目、補習練習，還是考前重點，都能整理成專屬練習場，隨時隨地，想測就測，系統自動對答案、統計成績、整理錯題，清楚知道自己哪裡還可以變更強！！',
    questions: [
      {
        id: 'sample-reading-1',
